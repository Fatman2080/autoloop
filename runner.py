...
def update_report(round_num: int, metrics: dict, strategy_code: str, backtest_output: str):
    """每当发现更优策略时，更新 REPORT.md"""
    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

    # 从回测输出中提取完整的训练集/验证集指标块
    train_block = re.search(r"训练集回测\n={50}\n(.*?)\n\n", backtest_output, re.DOTALL)
    val_block = re.search(r"验证集回测（优化目标）\n={50}\n(.*?)\n\n", backtest_output, re.DOTALL)
    train_text = train_block.group(1).strip() if train_block else "N/A"
    val_text = val_block.group(1).strip() if val_block else "N/A"

    # 从 strategy.py 提取 docstring 作为策略说明
    doc_match = re.search(r'"""(.*?)"""', strategy_code, re.DOTALL)
    docstring = doc_match.group(1).strip() if doc_match else "无"

    # 读取迭代历史
    results_tsv = read_file("results.tsv")
    kept_lines = []
    for line in results_tsv.strip().splitlines():
        if "保留" in line or "最佳" in line:
            kept_lines.append(f"| {line.replace(chr(9), ' | ')} |")

    code_lines = [l for l in strategy_code.splitlines()
                  if l.strip() and not l.strip().startswith("#") and not l.strip().startswith('"""')]

    report = f"""# AutoLoop 策略报告

> 自动生成于 {now} — 第 {round_num} 轮迭代
...
| 指标 | 训练集 | 验证集 |
|------|--------|--------|
| **Score** | {metrics.get('train_score', '?')} | **{metrics.get('val_score', '?')}** |
| **Sharpe Ratio** | {metrics.get('train_sharpe', '?')} | {metrics.get('val_sharpe', '?')} |
| **总收益** | {metrics.get('train_return', '?')}% | {metrics.get('val_return', '?')}% |
| **最大回撤** | {metrics.get('train_drawdown', '?')}% | {metrics.get('val_drawdown', '?')}% |
...
| 轮次 | 策略 | train_score | val_score | train_return | val_return | train_dd | val_dd | 状态 |
|------|------|-------------|-----------|--------------|------------|----------|--------|------|
{chr(10).join(kept_lines) if kept_lines else "| - | 暂无 | - | - | - | - | - | - |"}
...
def main():
    ...
    for rnd in range(start_round, start_round + max_rounds):
        ...
        if new_val_score > best_val_score:
            ...
            update_report(rnd, metrics, code, output)
        else:
