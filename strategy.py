...
改动说明 (Round 47):
- 保留 R20 核心框架（L25%/S40% + Kelt2.0x + EMA150斜率 + ADX>25）
- 优化做空进出场：使用更短的出场周期(28)替代原来的36，增加做空灵活性
- 优化仓位：做空从40%微调至35%，降低极端行情风险
- 简化代码：去除冗余变量，使用更高效的向量化计算
"""
...
def generate_signals(candles: pd.DataFrame) -> pd.Series:
    ...
    # ── 做空系统 (35% 仓位，熊市+ADX>25 时激活) ──
    exit_high = high.rolling(28).max()  # 改为28，与做多对称
    ...
    for i in range(150, len(candles)):
        ...
        if not in_short:
            if (bear_confirmed
                    ...
                    and volume.iloc[i] > 1.1 * vol_ma.iloc[i]):
                in_short = True
                short_signal.iloc[i] = -0.35  # 从0.4降至0.35
        else:
