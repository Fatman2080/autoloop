...
strategy.py — 量化交易策略
改动：优化做空系统参数
- Keltner 系数：2.0x → 1.8x（更紧密的通道，做空信号更灵敏）
- ADX 阈值：25 → 20（降低门槛，增加做空机会）
- 保持做多系统 25% 仓位不变
- 保持做空系统 40% 仓位不变
理由：R17 用更激进的 1.5x Keltner 获得 val=2.39，验证了收紧 Keltner 对做空系统的正向作用
"""
...
def generate_signals(candles: pd.DataFrame) -> pd.Series:
    ...
    # ── 做空系统（熊市 + ADX>20 + Keltner下轨1.8x，40% 仓位） ──
