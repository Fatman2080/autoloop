"""
strategy.py — 改进策略
改动：在 R20 基础上加入 RSI 过滤 + 小幅增加做空仓位
- RSI(14) < 30 作为做空额外确认，减少震荡市假信号
- 做空仓位从 40% 提升至 45%，更充分捕捉熊市趋势
- 保持 EMA150 熊市检测 + ADX>25 双重过滤
"""
...
def generate_signals(candles: pd.DataFrame) -> pd.Series:
    ...
    for i in range(150, len(candles)):
        ...
        rsi_oversold = rsi.iloc[i] < 40 if not np.isnan(rsi.iloc[i]) else False  # 新增

        if not in_short:
            if (bear_confirmed
                    and adx_strong
                    and rsi_oversold  # 新增 RSI 过滤
                    and close.iloc[i] < keltner_lower.iloc[i]
                    and volume.iloc[i] > 1.1 * vol_ma.iloc[i]):
                in_short = True
                short_signal.iloc[i] = -0.45  # 从 -0.4 提升至 -0.45
        else:
