...
strategy.py — 量化交易策略
基于R20最佳策略的改进：
- 保持独立多空系统 + EMA斜率熊市检测 + ADX趋势强度框架
- 调整：做空仓位从40%提升至50%，增加做空盈利潜力
- 调整：Donchian出场参数（做空36->40），延长持仓时间捕获更多趋势
- 调整：ADX阈值从25降至20，允许更多做空信号参与
- 原理：R20 val_score=1.870是历史最佳，需微调而非大幅改动
"""
...
def generate_signals(candles: pd.DataFrame) -> pd.Series:
    ...
    # ── 做空系统（熊市+ADX>20 强趋势，50% 仓位）──
    ema150 = close.ewm(span=150, adjust=False).mean()
    ema150_slope = ema150 / ema150.shift(96) - 1
    exit_high = high.rolling(40).max()  # 从36改为40，延长持仓
    ...
    for i in range(150, len(candles)):
        ...
        adx_strong = adx.iloc[i] > 20 if not np.isnan(adx.iloc[i]) else False  # 从25降至20
        ...
        if not in_short:
            if (bear_confirmed
                    ...
                    and volume.iloc[i] > 1.1 * vol_ma.iloc[i]):
                in_short = True
                short_signal.iloc[i] = -0.5  # 从-0.4提升至-0.5
        else:
