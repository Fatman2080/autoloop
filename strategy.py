"""
strategy.py — 量化交易策略
改动：降低ADX阈值(25→20)增加做空机会，加强成交量确认(1.1x→1.2x)过滤假突破
保持：EMA150斜率熊市检测 + 40%做空仓位 + Keltner(2.0x)
"""
...
def generate_signals(candles: pd.DataFrame) -> pd.Series:
    ...
    for i in range(150, len(candles)):
        ...
        adx_strong = adx.iloc[i] > 20 if not np.isnan(adx.iloc[i]) else False  # 阈值降至20

        if not in_short:
            if (bear_confirmed
                    and adx_strong
                    and close.iloc[i] < keltner_lower.iloc[i]
                    and volume.iloc[i] > 1.2 * vol_ma.iloc[i]):  # 成交量确认加强至1.2x
                in_short = True
