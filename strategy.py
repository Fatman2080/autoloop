...
- 出场窗口从36调整为44，延长空头持仓时间以捕捉更大下跌趋势
- 做空仓位从70%微调至65%，略微降低仓位以减少极端行情波动
- 保持Keltner 2.5x + EMA150熊市检测 + ADX>25 + 成交量1.1x确认

预期：更长持仓时间 + 适度仓位 → 提升Sharpe同时控制回撤
"""
    ...
    for i in range(150, len(candles)):
        ...
        if not in_short:
            if (bear_confirmed
                    ...
                    and volume.iloc[i] > 1.1 * vol_ma.iloc[i]):
                in_short = True
                short_signal.iloc[i] = -0.65  # 从-0.70调整为-0.65
        else:
