...
改进说明（R56 → R57）：
- 在R20(最佳)基础上，将做多出场从固定28周期改为：min(Donchian低, ATR动态止损)
- ATR止损逻辑：exit_low - 1.5 * atr_lookback，作为动态保护
- 这样在趋势延续时让利润奔跑，在快速回调时及时止损
- 保持其他所有参数不变，避免过拟合风险
"""
...
def generate_signals(candles: pd.DataFrame) -> pd.Series:
    ...
    exit_low_donchian = low.rolling(28).min()
    
    # ATR动态止损：取28周期最低点减去1.5倍ATR
    atr_lookback = 28
    atr_protection = exit_low_donchian - 1.5 * atr.rolling(atr_lookback).mean()
    # 实际出场：取两者先触发（更早止损）
    exit_low = pd.concat([exit_low_donchian, atr_protection], axis=1).min(axis=1)
