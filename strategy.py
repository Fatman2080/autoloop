...
- EMA150斜率阈值从 -0.05 调整为 -0.03（放宽条件，增加做空信号）
- 出场窗口从 36 缩短到 30（提高资金周转）
- 保持做空仓位 70%，Keltner 2.5x，ADX>25，成交量 1.1x 确认

预期：更多做空机会 + 更高周转 → 提升 val_score
"""
...
def generate_signals(candles: pd.DataFrame) -> pd.Series:
    ...
    for i in range(150, len(candles)):
        ...
        if np.isnan(slope):
            slope = 0.0
        bear_confirmed = close.iloc[i] < ema150.iloc[i] and slope < -0.03  # 从-0.05调整为-0.03
