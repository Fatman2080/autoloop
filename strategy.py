...
改动说明（R21）：
- Keltner宽度从2.0x扩大至2.5x，减少震荡市假突破
- 做空ADX阈值从25提升至30，进一步过滤弱趋势
- 仓位微调：做多维持25%，做空从40%降至35%
- 熊市检测：EMA150斜率阈值从-5%收紧至-6%
"""
...
def generate_signals(candles: pd.DataFrame) -> pd.Series:
    ...
    for i in range(150, len(candles)):
        ...
        if np.isnan(slope):
            slope = 0.0
        # 熊市判定更严格：斜率<-6%
        bear_confirmed = close.iloc[i] < ema150.iloc[i] and slope < -0.06
        # ADX阈值提升至30
