...
strategy.py — 量化交易策略
改进方向：
1. Keltner 通道改用 EMA100（原来 EMA50）- 更稳定的趋势判断
2. ATR 倍数从 2.0 提升到 2.5 - 更严格的入场条件，减少假突破
3. 做空入场增加 ADX>30 过滤（原 ADX>25）- 更严格的趋势强度要求
4. 仓位微调：做多 30%，做空 35%（原 25%/40%）

核心逻辑保持不变：独立叠加多空系统 + EMA 斜率熊市检测 + ADX 趋势强度
"""
...
def generate_signals(candles: pd.DataFrame) -> pd.Series:
    ...
    # ── Keltner 通道 (EMA100 + 2.5x ATR) ──
    keltner_upper = ema100 + 2.5 * atr
    keltner_lower = ema100 - 2.5 * atr
