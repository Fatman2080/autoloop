...
改进说明（基于R117 val_score=2.0536）：
- 做空仓位从70%降到65%，避免极端重仓
- 出场窗口从36降到28，更快止盈
- 新增时间止损：空头持仓超过24根K线强制平仓，防止趋势反转利润回吐
- 保持：Keltner 2.5x + EMA150熊市检测 + ADX>25 + 成交量1.1x确认

预期：时间止损保护利润 + 适中做空仓位 → 提升 Sharpe
"""
...
def generate_signals(candles: pd.DataFrame) -> pd.Series:
    ...
    # ── 做空系统（EMA150熊市 + ADX强趋势 + 时间止损，65% 仓位） ──
    ...
    for i in range(150, len(candles)):
        ...
        if not in_short:
            if (bear_confirmed
                ...
                in_short = True
                short_bars = 0
                short_signal.iloc[i] = -0.65
        else:
            short_bars += 1
            # 时间止损：持仓超过24根K线强制平仓
            time_stop = short_bars >= 24
            price_exit = close.iloc[i] > exit_high.iloc[i - 1]
            
            if time_stop or price_exit:
                in_short = False
                short_bars = 0
            else:
                short_signal.iloc[i] = -0.65
