...
- 出场窗口从36调整为34，更快出场以减少回撤
- EMA150斜率阈值从-0.05调整为-0.04，放宽熊市条件以增加做空信号
- Keltner倍数保持2.5x，成交量确认1.1x，做空仓位70%

预期：更快的出场 + 更敏感的熊市检测 → 提升Sharpe同时控制回撤
"""
...
def generate_signals(candles: pd.DataFrame) -> pd.Series:
    ...
    exit_high = high.rolling(34).max()  # 从36调整为34，更快出场
    ...
    for i in range(150, len(candles)):
        ...
        if np.isnan(slope):
            slope = 0.0
        bear_confirmed = close.iloc[i] < ema150.iloc[i] and slope < -0.04  # 从-0.05调整为-0.04
