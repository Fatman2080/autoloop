...
改进点：
- 做空仓位：40% → 50%
- 做空出场通道：36h → 24h（与做多系统一致，加速出场减少回撤）

原理：R75达到val_score=2.3323，L30%/S50%仓位配比有效；缩短出场通道可减少趋势反转时的亏损。
"""
...
def generate_signals(candles: pd.DataFrame) -> pd.Series:
    ...
    # ── 做空系统（熊市+ADX>25，50%仓位，出场24h） ──
    ema150 = close.ewm(span=150, adjust=False).mean()
    ema150_slope = ema150 / ema150.shift(96) - 1
    exit_high = high.rolling(24).max()  # 改进：36h → 24h
    ...
    for i in range(150, len(candles)):
        ...
        if not in_short:
            if (bear_confirmed
                    ...
                    and volume.iloc[i] > 1.1 * vol_ma.iloc[i]):
                in_short = True
                short_signal.iloc[i] = -0.5  # 改进：-0.4 → -0.5
        else:
