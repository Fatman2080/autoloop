...
- Keltner从2.5x收窄到1.8x（接近R17的1.5x），降低入场门槛增加交易机会
- 做空仓位从70%微调到55%（更平衡，避免过度做空）
- 出场窗口从36调整为42（延长持仓）
- 新增RSI<40超卖过滤（提高做空入场质量）

预期：更低的Keltner门槛 + RSI过滤 → 增加有效交易 + 提升val_score
"""
...
def generate_signals(candles: pd.DataFrame) -> pd.Series:
    ...
    # ── 做空系统（EMA斜率熊市 + ADX强趋势 + RSI过滤，55% 仓位） ──
    ema150 = close.ewm(span=150, adjust=False).mean()
    ema150_slope = ema150 / ema150.shift(96) - 1
    exit_high = high.rolling(42).max()  # 从36调整为42
    ...
    for i in range(150, len(candles)):
        ...
        rsi_oversold = rsi.iloc[i] < 40 if not np.isnan(rsi.iloc[i]) else False
        ...
        if not in_short:
            if (bear_confirmed
                    ...
                    and volume.iloc[i] > 1.1 * vol_ma.iloc[i]):
                in_short = True
                short_signal.iloc[i] = -0.55  # 从-0.70调整到-0.55
        else:
