...
改进说明（R56）：
- 目标：在R20基础上提高val_score稳健性
- 改动1：做空仓位 40% → 30%（减少单次亏损）
- 改动2：做空出场通道 36 → 48（减少频繁止损）
- 理由：R17曾达到val_score 2.3948(L30%/S20%)，当前S40%可能偏大
"""
...
def generate_signals(candles: pd.DataFrame) -> pd.Series:
    ...
    # ── 做空系统（仅在 EMA斜率熊市 + ADX强趋势 中激活，30% 仓位） ──
    ema150 = close.ewm(span=150, adjust=False).mean()
    ema150_slope = ema150 / ema150.shift(96) - 1
    # 改动2：出场通道 36 → 48，减少频繁止损
    exit_high = high.rolling(48).max()
