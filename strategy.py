...
改动说明：
- 基础: R20 (val_score=1.870, test:+3.89%)
- 核心改进: Keltner倍数从2.0x降至1.5x
  - R17曾用1.5x达到val_score=2.3948（虽训练集亏损，但验证集表现优异）
  - 更紧密的轨道意味着更早捕捉趋势，减少滞后
- 保持: 熊市检测 + ADX>25 过滤 + 成交量确认
- 仓位: 做多25% + 做空40% 保持不变

预期: 在R20基础上进一步提升验证集 Sharpe
"""
