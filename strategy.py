...
strategy.py — 量化交易策略
改动说明：
- ADX阈值从25降至20：捕捉更多中等级别趋势，减少"趋势早期"踏空
- 做空出场从36减至30：缩短止损周期，降低回撤
- 做空仓位从40%提升至45%：在强趋势中获取更高收益
- 保持EMA150斜率熊市检测+Keltner 2.0x的核心架构
"""
...
def generate_signals(candles: pd.DataFrame) -> pd.Series:
    ...
    # ── 做空系统（熊市 + ADX>20 强趋势，45% 仓位）────
    ema150 = close.ewm(span=150, adjust=False).mean()
    ema150_slope = ema150 / ema150.shift(96) - 1
    # 改动：出场从36减至30
    exit_high = high.rolling(30).max()
