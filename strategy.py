...
改进点（R33）：
- 做空出场：从固定 Donchian 高点(36) 改为 ATR 追踪止损
- ATR 追踪止损 = 入场价 - 2.5 * ATR(14)
- 理由：固定高点出场在震荡市容易被假突破打掉，ATR 止损更贴近波动率，
  能更早退出又能抓住趋势。保持其他参数不变确保可对比。
    ...
    for i in range(150, len(candles)):
        ...
        if not in_short:
            if (bear_confirmed
                ...
                atr14_val = atr14.iloc[i] if not np.isnan(atr14.iloc[i]) else atr.iloc[i]
                short_signal.iloc[i] = -0.4
        else:
            # ATR 追踪止损出场
            stop_loss = short_entry_price - 2.5 * atr14_val
