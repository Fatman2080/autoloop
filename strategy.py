...
基于R159改进（val_score=2.5679）：
- 调整做空ATR动态出场：2.8×ATR → 3.0×ATR，进一步放宽出场让利润更奔跑
- 保持波动率自适应仓位机制
- 保持EMA150熊市检测 + ADX>25 + Keltner 2.5x + 成交量1.1x

改进理由：
从历史数据发现，ATR动态出场从2.5→2.8已验证有效(R138-R159系列)，
继续放宽到3.0x让做空趋势行情获取更多利润，同时保持总仓位在合理范围。
"""
