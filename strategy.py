...
基于R279(val_score=3.8058)的改进：
1. 当前策略：做空仓位75% + 85%部分止盈
2. 改进：将做空仓位从75%提升到80%
3. 理由：历史证明提高做空仓位持续有效
   - R275: 55%→3.6267
   - R276: 60%→3.6759
    ...
    for i in range(150, len(candles)):
        ...
        if not in_short:
            if (bear_confirmed
                ...
                partial_closed = False
                short_signal.iloc[i] = -0.80 * vol_mult.iloc[i]  # 从0.75提高到0.80
        else:
            # 检查是否触发部分止盈（价格有利移动2.5x ATR）
            if not partial_closed and close.iloc[i] <= entry_price - atr_take_profit.iloc[i]:
                # 平掉85%仓位，剩余15%仓位（0.80 * 0.15 = 0.12）
                short_signal.iloc[i] = -0.12 * vol_mult.iloc[i]
                partial_closed = True
                ...
                # 继续持有剩余仓位
                if partial_closed:
                    short_signal.iloc[i] = -0.12 * vol_mult.iloc[i]  # 剩余15%仓位
                else:
