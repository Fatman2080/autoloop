...
- 调整做空ATR动态出场：2.8×ATR → 2.9×ATR，进一步让利润奔跑
- 调整做空仓位：60% → 65%（适度增加做空获利的仓位）
- 保持：EMA150熊市检测 + ADX>25 + Keltner 2.5x + 成交量1.1x + 波动率自适应

改进思路：
历史数据显示，ATR动态出场从2.5x→2.8x有效，2.9x可能进一步优化；
做空仓位从60%→65%可增加在确认的熊市中的盈利机会。
"""
...
def generate_signals(candles: pd.DataFrame) -> pd.Series:
    ...
    # ── 做空系统（65% 仓位 × 波动系数，ATR动态出场） ──
    ema150 = close.ewm(span=150, adjust=False).mean()
    ema150_slope = ema150 / ema150.shift(96) - 1
    
    # ATR动态出场：2.9×ATR，让利润更奔跑
    atr_exit = atr14 * 2.9
